  ...
  methods: {
    consultarCriterios() {
      ...
      axios
        .get(url)
        .then((response) => {
          if (response.data.arrayCriteria) {
            this.criterios = response.data.arrayCriteria;
          } else {
            console.error('La respuesta del servidor no contiene el campo "arrayCriteria".', response.data);
          }
        })
        .catch((error) => {
    ...
    this.$router.push({ name: 'editarcriterio', params: { serviceIdCriteria: idService, standardIdCriteria: idStandard, idCriteria: id } });
