          ...
          <tbody>
            <tr v-for="servicio in paginatedServicios" :key="servicio.id">
              ...
              <td>
                ...
                <button @click="Criterios(servicio.id)">Criterios</button>
              </td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  methods: {
    consultarServicios() {
      ...
      const url = `https://redb.qsystems.co/QS3100/QServlet?operation=queryServiceByTenancy&tna=${tna}&key=${key}`;

      axios
        .get(url)
        .then((response) => {
          if (response.data.arrayService) {
            this.servicios = response.data.arrayService;
          } else {
            console.error('La respuesta del servidor no contiene el campo "arrayService".', response.data);
          }
        })
        .catch((error) => {
    ...
    Criterios(id) {
      ...
      this.$router.push({ name: 'listarcriterioservicio', params: { idService: id } });
    },
