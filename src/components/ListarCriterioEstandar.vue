  ...
  export default {
    ...
    methods: {
      ...
      fetch(`https://redb.qsystems.co/QS3100/QServlet?operation=queryCriteriaByStandard&tna=5&key=e35d751c-12a8-4789-91d0-a95f055f0630&standardIdCriteria=${this.standardData.idStandard}`, {
        method: 'GET',
      })
    ...
    async consultarCriterioPorId(idCriteria) {
        try {
          ...
          if (response.data.arrayCriteria && response.data.arrayCriteria.length > 0) {
            // Obtén el idStandard directamente del criterio
            const idStandard = response.data.arrayCriteria[0].standardID;
  
            if (idStandard) {
              this.editCriterio(idCriteria, idStandard);
            } else {
              console.error('No se encontró idStandard en la respuesta del servidor.');
            }
          } else {
      ...
      editCriterio(id, idStandard) {
        console.log('editCriterio method called with idStandard:', idStandard, 'and id:', id);
        this.$router.push({ name: 'editarcriterioestandar', params: { standardIdCriteria: idStandard, idCriteria: id } });
